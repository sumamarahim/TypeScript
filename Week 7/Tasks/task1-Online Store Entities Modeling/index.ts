...
// Cart type as an array of Product object
type Cart = Product[];
...
// Order type and stored customer and cart value
type Order = {
    customer: Customer,
    cart: Cart,
}
...
function calculateTotalPrice(cart: Cart): number {
  ...
  const customer: Customer = { id: 1, name: "Ali", email: "info@sumahim.com" };
  
  const order: Order = { customer, cart: products };
  
  const totalPrice = calculateTotalPrice(order.cart);
  console.log("Customer ",customer.name,
  " using email ",customer.email,
  " have Total Price:",
   totalPrice," for products: ",products);
