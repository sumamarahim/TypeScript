...
console.log("are all elements less than 10: ", Arr.every((x) => x < 10));
console.log("are all elements greater than 0: ", Arr.every((x) => x > 0));
console.log("are ", Arr.every((Arr) => Arr));
console.log("are all elements less then 0: ", Arr.every((b) => b < 0));
// ========================================
// some: finding if some elements falls under specific/defined condition
console.log("Checking for elements if some of them are below 5: ", Arr.some((x) => x < 5));
console.log("Checking for elements if some of them are above 5: ", Arr.some((x) => x > 5));
console.log("Checking for elements if some of them are below 0: ", Arr.some((x) => x < 0));
console.log("Checking for elements if some of them are above 10: ", Arr.some((x) => x > 10));
// ==========================================
// at:
...
console.log("Finding all even num: ", arr11.filter((y) => y % 2 == 0));
console.log("Finding all odd num: ", arr11.filter((y) => y % 2 == 1));
// =================================================
// find: returns first element if condition met
let findArr = [1, 2, 3, 4, 5, 6, 7, 8];
console.log("first number before 4 in array is: ", findArr.find((x) => (x < 4 ? "good" : "bad")));
console.log("first number after 4 is: ", findArr.find((x) => x > 4));
console.log("first even number in array is: ", findArr.find((x) => x % 2 == 0));
console.log("first even number in array is: ", findArr.find((x) => x % 2 == 1));
let findArr1 = [3, 4, 6, 7, 9];
console.log("first even number in array[3, 4, 6, 7, 9] is: ", findArr1.find((y) => y % 2 == 0));
// ============================
// reduce: used to apply for a function against two values to reduce and make it one value
let newArr = [1, 2, 5, 3, 7, 8];
console.log("reduce method [1, 2, 5, 3, 7, 8] adds all", newArr.reduce((x, y) => x + y));
console.log("reduce method [1, 2, 5, 3, 7, 8] minus all", newArr.reduce((x, y) => x - y));
console.log("reduce method [1, 2, 5, 3, 7, 8] mul all", newArr.reduce((x, y) => x * y));
console.log("reduce method [1, 2, 5, 3, 7, 8] div all", newArr.reduce((x, y) => x / y));
// etc
////////////////////////////////
// map
let mapArr = [3, 4, 6, 8, 9];
console.log('map method to multiply all elements with any number, let say 3:', mapArr.map((x) => x * 3));
console.log('add 2 to all ele:', mapArr.map((x) => x + 2));
console.log('div with 2 of all ele:', mapArr.map((x) => x / 2));
console.log('mul with 2 of all ele:', mapArr.map((x) => x * 2));
// etc
////////////////////////////////////////////
// findIndex
let findInd = [2, 3, 6, 7, 9];
let findInd1 = [2, 4, 3, 5];
console.log('findIndex: ', findInd1.findIndex((x) => x % 2 !== 0));
