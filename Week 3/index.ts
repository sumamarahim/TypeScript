...
const promptSync = require("prompt-sync")(); // import prompt sync 
...
const prompt1 = require("prompt-sync")()("Enter Name again: ");
console.log(`Output is: ${prompt1}`);
...
let myName: string = "Nauman";
console.log(
  "Hello" + " " + myName + "," + " would you like to learn some Python today?"
);
//   ===========================

// 3. Name Cases: Store a person’s name in a variable, and then print that person’s name in lower, upper & titlecase.
let nameCase: string = "Nauman Anwar";
...
console.log(
  `Albert Einstein once said, “A person who never made a mistake never tried anything new.”`
); // Escaped Characters with single Quote.
...
let myNum: number = 47;
let myMsg: string = `Hello, my favourite number is ${myNum}`; // ${variable} method allows us to insert a variable value inside string
console.log(myMsg);
...
const friendNames: string[] = ["Adnan", "Jalal", "Danial", "Arsalan", "zahoor"];
...
let guests: string[] = ["Adnan", "Danial", "Nabeel"];

guests.forEach(function (arr) {
  // The forEach() method calls a function once for each array element.
  console.log(
    "Greetings! " + arr + ", I cordially invite you to my birthday party."
  );
});
...
let newGuests: string[] = ["Inayat", "Jalal", "Zaman", "Omar"];
newGuests.forEach(function (frnd) {
  console.log(
    "I have a new Friend" + " " + frnd + " " + "to invite to my party."
  );
});
...
guests.forEach(function (frnd) {
  console.log(
    "Greetings! " + frnd + ", I cordially invite you to my birthday party."
  );
});
...
guests.forEach(function (friend) {
  console.log(
    "Greetings! " + friend + ", I cordially invite you to my birthday party."
  );
});
...
let guests1: string[] = ["Adnan", "Danial", "Nabeel"];
var numGuest: number = 0;
...
const myObject: { Mountains: string; Rivers: string; Countries: string } = {
  Mountains: "k2",
  Rivers: "Indus",
  Countries: "Pakistan",
};
...
let ourString: string = "Bus";
console.log(
  "comparison with AND operator will always evaluate to false because a single variable cannot hold two different values at the same time, therefore it is throwing an error.\n"
);
...
const cars = ["Ford", "Toyota", "GMC", "Honda"];
...
var favourite_fruits: string[] = ["Apple", "Mango", "Cherry"];
...
let adminArr: string[] = ["admin", "user1", "user2", "user3", "user4"];
...
let new_users: string[] = ["Nauman97", "JOHN", "Fayyaz", "Saleem", "Ali"];
...
// }
for (var i = 0; i < new_users.length; i++) {
  let lowerCaseUser = new_users[i].toLowerCase();
  if (current_users.includes(new_users[i])) {
...
let myPizza: string[] = ["Fajita", "Chicken Supreme", "Pepperoni"];
...
const myAnimals: string[] = ["Cat", "Dog", "Parrot"];
...
for (var i = 0; i < myAnimals.length; i++) {
  if (myAnimals[i] == "Cat") {
    console.log(`${myAnimals[i]} is such a calm pet.`);
  } else if (myAnimals[i] == "Dog") {
    console.log(`${myAnimals[i]} is such a playful pet.`);
  } else {
    console.log(`${myAnimals[i]} mimics everyone.`);
  }
}
...
function make_shirt(size: number, msg: string) {
  ...
  let myTxt: string = msg;
  return `The size of this shirt is ${Mysize}.\nThe text to be printed on the shirt is: ${myTxt}.`;
}
...
function new_shirt(
  size: string = "large",
  txt: string = "Standing on the shoulder of giants"
) {
  return `The size of this shirt is ${size}.\nThe text to be printed on the shirt is: ${txt}.`;
}
...
function make_album(name: string, title: string, trackNum: number = 0) {
  let myObject: any = {};
...
function magician_Name(arr: string[]) {
  for (var i = 0; i < arr.length; i++) {
    console.log(arr[i]);
  }
}
let Arr2: string[] = ["David Copperfield", "Harry", "Criss Angel", "Dymano"];
magician_Name(Arr2);
...
let modifiedArray = new_arr(Arr2);
console.log(modifiedArray);
