...
function welcome(useName: string): void
{
...
function addNums1(num1: number, num2: number)
{
    let a = 1;
    let b = 3;
...
function addWithIandO(nums1: number, nums2: number)
{
...
function toCelsius(fahrenheite: number): number
{
...
let xy = function (a: number, b: number)// this is anonymous function which is without name
{
    return a * b;
}
...
let sumNum = (x: number, y: number): number => {
    return x + y;
}
console.log(sumNum(1,3));
let sumNum01 = (x: number, y: number) => console.log(x+y);
sumNum01(2,3);
// THe above and this both are same
// sumNum01 and sumNum02 are same 
function sumNum02(x: number, y: number) {
    return console.log(x + y);
}
sumNum02(2,3);

let sumNum1 = (x: number, y: number): number => x+y;
...
let addition2 = (num1: number, num2: number) => num1 + num2;
addition2(12, 19);
